...
\geometry{a4paper,body={7.45in,10.2in}}
...
% STRIPS is however quite limited in the complexity of the problems it can represent, as it cannot effectively 
% represent non-boolean value such as positions. The current standard for planning domain representation, PDDL, 
% is more expressive, being able to represent types, numbers and different action costs. 

Despite the advent of more expressive planning domains, such as PDDL, applications of STRIPS are still being researched,
...
which is more expressive, being able to represent types, numbers and action costs \cite{aeronautiques1998pddl}.
Still, the impact of STRIPS on the field of planning can be seen here,
as the PDDL standard is heavily inspired by STRIPS and even specifies a STRIPS-only mode.

The Graphplan algorithm, introduced by Blum et al. in 1997, solves problems from the STRIPS domain
not by exploring the state graph, but by constructing a planning graph, a datastructure that efficiently encodes which
propositions and actions are possible to reach and which are mutually exclusive \cite{blum1997fast}. 
Because the complexity is not directly bound to the size of the state graph, the algorithm scales extremely well 
on many different problems (see also \ref{sec:results}) \cite{blum1997fast}. 

Due to the additional complexity with formal verification of algorithms, there are only few works on formally verifying 
planning algorithms; notably, Abdulaziz and Koller wrote a formally verified validation for the temporal domain 
specified in the PDDL standard and Abdulaziz and Kurz wrote a formally verified translator from the planning domain 
to the problem of satisfiability, allowing the usage of SAT-Solvers to solve planning problems \cite{abdulaziz2023formallyverifiedsatbasedai} \cite{Abdulaziz_Koller_2022}.
However, there is one paper by Abdualaziz et al. where a compositional Algorithm for a problem equivalent to STRIPS 
is formally verified in the HOL4 interactive theorem prover \cite{abdulaziz2019verified}. 

\section{STRIPS}
\label{sec:strips}

\subsection{The STRIPS domain}
\label{subsec:problem_space}

%TODO: look at the solver the paper used and compare it to linear search and graphplan
The STRIPS domain is a closed world of first-order predicate logic.
...
\begin{figure*}
  ...
  \caption{The planning graph of Graphplan on the rocket problem. Configuration: One Rocket, 
  two pieces of cargo, A and B, one start Location L, one destination R; from Blum et. al. 1997. }
\end{figure*}
...
The first action is "Load", with three free variables that have to implement the predicates "Cargo", "Rocket" and "Place" respectively.
...
A simple breadth-first search does work as a reference for solving simple problems. % And was also implemented for this paper ?
...
planning graph and searching it instead. The planning graph that the algorithm creates during the search phase alternates between
...
\subsection{Functionality; Algorithm}
...
in the state layer such that no pair of preconditions have a mutex. Additionally, all pairs of distinct actions
...
Lean is a programming language and proof assistant that 
...
auto-formalization of mathematical problems \cite{leanPage} \cite{alphaProof} \cite{tao2025aiContributions}.
...
For this paper, Lean was used both to implement STRIPS
as well as the search algorithms, including Graphplan.
...
which is inhabited iff the corresponding proposition is true. Proving a proposition in Lean thus
...
Testing the implementation shows it to be able to solve small problems within reasonable time, but quickly slows down once the problem becomes bigger, 
...
and instead, before returning the result, checked whether it was correct. This could then be returned as 
a proof of the correctness, returning "no result found" if it was not correct.
A proper solution that proves that the backtracking can only result in a proper solution without having to check, 
was later written manually, leaving less than a fourth of the code generated by Aristotle. 

Note that the proof of correctness is only for the result that is returned, not for the entire algorithm, in that 
it was not %TODO remove?
proven that the algorithm always returns a correct solution if one exists, only that if it does return a solution, it is correct.
...
\begin{figure}
  ...
  \caption{ Performance of four different algorithms on the Rocket problem
...
\begin{figure}
  \label{Figure 3}
  \centering
  \includegraphics[width = 240pt]{Performance_on_SlidingN.png}
  \caption{ Performance of four different algorithms on the 15-Puzzle
  with n random moves from a solved state, in ms}
\end{figure}
...
In the second experiment, the 15-Puzzle is encoded in STRIPS with the 15 sliding pieces numbered one to 15 and
the cells of the four-by-four grid being numbered one to 16. There is a single action, to slide one of the pieces into the empty
cell, swapping them. The experiment shown in Figure 3 gives as input the 15-Puzzle with $n$ random moves from the solved state. 
While the seach space is very large with approximately $10^{13}$ positions, it has a maximal branching factor of 4, as a 
maximum of four pieces could be moved in a single position \cite{culberson1994efficiently}.
Notably, as can be seen in Figure 3, the Graphplan algorithm scales much better than the algorithms which explore the state space. 
Additionally, the best first search takes more than six minutes for solving six random moves because it encounters
a sussmann anomaly where placing two pieces (seven and eight) in their final positions greedily leads to a wrong parity 
in the last three pieces (eleven, twelve and fifteen). The other algorithms do not encounter this anomaly because they 
do not search greedily. 

Both experiments was performed on a Macbook with M1 chip while it was connected to power, 
with Lean running the compiled algorithms on the problem 100 times.
All datapoints higher than 500ms were experiments that were only run once.
All algorithms expect Graphplan were stopped after running more than five minutes without finding a solution.
...
This paper presented a formal implementation of the STRIPS planning domain in the Lean programming language and theorem prover, 
as well as an implementation of the Graphplan Algorithm to solve STRIPS instances.
Two experiments on STRIPS problems with variable complexity were run to measure the performance of Graphplan 
as well as three other reference algorithms. 
In both cases, the Graphplan algorithm overtook all other algorithms in speed as the complexity increased.
Additionally, the correctness of the result of a breadth-first search algorithm was proven formally, demonstrating
the ability of Lean to express correctness of planning algorithms. 
